...
- **Premium Frontier Models & BYOK Modal (`MessageInput.tsx`, `PremiumModelModal.tsx`, `modelUtils.ts`)**: Partitioned model selection into **Standard Models** and **✦ Premium Frontier Models** (with refined `PRO` badges and lock indicators). Clicking any premium model prompts users via a sleek glassmorphic modal with a direct recommendation to connect their personal API key in Settings (BYOK) and a dummy **"Pro Plan (Coming Soon)"** upgrade option with graceful fallback to select anyway.
