...
/**
 * Normalizes provider keys so that aliases (e.g. gemini_free, google)
 * map to a canonical single provider name (e.g. gemini).
 */
export function normalizeProviderName(provider: string): string {
  if (!provider) return "";
  const p = provider.toLowerCase().trim();
  if (p === "gemini_free" || p === "gemini-free" || p === "google") return "gemini";
  if (p.endsWith("_free")) return p.replace(/_free$/, "");
  return p;
}
...
export function isPremiumModel(
  ...
  // 1. Explicitly designated by backend premium_models list
  if (providerData?.premium_models && providerData.premium_models.length > 0) {
  ...
  if (providerData && providerData.is_free === false && !providerData.is_byok_configured) {
    return true;
  }
