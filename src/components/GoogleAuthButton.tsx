...
export default function GoogleAuthButton({
  ...
  // Initialize GSI (for the credential callback), but do NOT render a button overlay
  useEffect(() => {
  ...
  // Primary click handler: open Google OAuth popup directly from our origin
  const handleClick = useCallback(() => {
    if (!clientId || disabled) return;

    // Try GSI prompt first (if available and called from user gesture, it may work)
    if (window.google?.accounts?.id?.prompt) {
      try {
        window.google.accounts.id.prompt();
        // If prompt works, the callback in initialize() handles the credential
        // If it silently fails, the direct popup below catches it
      } catch {
        // GSI prompt failed, continue to direct popup
      }
    }

    // Direct OAuth popup fallback (works in ALL browsers including Helium, Brave, Arc)
    // Opens from our own origin = trusted user gesture = never blocked
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative flex h-11 w-full items-center justify-center gap-3 rounded-input border border-black/20 bg-gradient-to-b from-[#e5ff33] via-primary to-[#b8da00] px-4 text-sm font-black text-background transition-all duration-200 ease-out ${isPressed
      ...
      >
        ...
        <div
          className={`pointer-events-none absolute inset-x-0 top-0 h-[48%] rounded-t-input bg-gradient-to-b from-white/40 via-white/15 to-transparent transition-opacity duration-200 ${isHovered ? "opacity-100" : "opacity-50"
            }`}
        />
        ...
        {/* Google Icon Badge */}
        <div
          className={`relative z-10 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white p-1 shadow-md ring-1 ring-black/15 transition-transform duration-200 ${isPressed ? "scale-95" : isHovered ? "scale-110 rotate-6" : "scale-100"
            }`}
        >
          <svg className="h-4 w-4" viewBox="0 0 24 24" aria-hidden>
            <path
